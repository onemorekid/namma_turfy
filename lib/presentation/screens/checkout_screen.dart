...
class _CheckoutScreenState extends ConsumerState<CheckoutScreen> {
  ...
  Future<void> _applyPromo() async {
    ...
    final coupon = await ref.read(venueRepositoryProvider).getCouponByCode(code);

    if (coupon == null) { _showError('Invalid coupon code'); return; }
    if (coupon.validTo.isBefore(DateTime.now())) { _showError('Coupon has expired'); return; }

    if (coupon.restrictedEmails != null && coupon.restrictedEmails!.isNotEmpty) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Coupon $code applied!'), backgroundColor: Colors.green),
    );
  }
  ...
  Future<void> _startPayment() async {
    ...
    if (!locksValid) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('Slot hold expired (10 min). Please select slots again.'),
  ...
  void _onPaymentSuccess(
    ...
    try {
      ...
      final bookingId =
          (Map<String, dynamic>.from(result.data as Map))['bookingId'] as String;
  ...
  void _onPaymentError(int? code, String? message) {
    if (!mounted) return;
    setState(() { _isProcessing = false; _pendingOptions = null; });
            ...
            Card(
              child: Padding(
                ...
                child: Column(children: [
                  ...
                  const Divider(),
                  ...widget.slots.map((s) => _SummaryRow(
        ...
        child: SafeArea(
          child: ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(
              ...
              backgroundColor:
                  orderReady ? const Color(0xFF1E88E5) : const Color(0xFF35CA67),
            ),
            child: _isProcessing
                ? const SizedBox(
                    height: 24,
